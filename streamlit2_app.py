...
st.markdown("""
    ...
    /* --- ส่วนที่ใช้ซ่อนเมนู GitHub และ Toolbar มุมขวา/ล่าง --- */
    ...
    /* KPI Card ปรับตัวเลขให้ใหญ่และชัดเจนพิเศษ */
    .kpi-card {
    ...
    .kpi-label { color: #64748b; font-size: 1.3rem; font-weight: 700; text-transform: uppercase; }
    .kpi-value { 
        color: #0f172a; 
        font-size: 5rem; /* ขยายขนาดตัวเลข */
        font-weight: 800; 
        margin: 0.5rem 0; 
        line-height: 1;
    }
    .kpi-delta-box { 
        padding: 12px 24px; border-radius: 50px; display: inline-block; 
        font-weight: 800; font-size: 1.6rem; 
    }

    .stButton>button {
        border-radius: 15px; background: #0f172a; color: white;
        font-weight: 800; border: none; padding: 1.2rem 2rem; width: 100%;
        font-size: 1.2rem;
    }
    </style>
...
if file_1 and file_2:
    ...
    if page == "📊 Executive Comparison":
        ...
        st.subheader("📊 เปรียบเทียบจำนวนวันรายวอร์ด (ตัวเลขชัดเจน)")
        fig_bar = px.bar(df_compare, x='Ward', y=['Total_Days_M1', 'Total_Days_M2'],
                         barmode='group', text_auto=',.0f',
                         color_discrete_sequence=['#cbd5e1', '#0f172a'], # ปรับสีให้ Contrast สูงขึ้น
                         labels={'value': 'จำนวนวัน (Days)', 'variable': 'เดือน'})
        fig_bar.update_layout(font=dict(size=18, family="Sarabun", color="#000"), plot_bgcolor='rgba(0,0,0,0)', 
                              paper_bgcolor='rgba(0,0,0,0)', legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
        fig_bar.update_traces(textposition='outside', textfont_size=18, textfont_weight="bold", textfont_color="#000")
        ...
        buf_comp = io.BytesIO()
        df_compare.drop(columns=['Status'] if 'Status' in df_compare.columns else []).to_excel(buf_comp, index=False)
