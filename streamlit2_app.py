...
st.markdown("""
    ...
    html, body, [class*="css"] { font-family: 'Sarabun', sans-serif; color: #000000; }
    
    .stApp { background-color: #ffffff; }
    
    /* หัวข้อหลักตัวใหญ่ชัดเจน */
    .main-title { 
        color: #0f172a; font-size: 3.5rem; font-weight: 800; 
        text-align: center; margin-bottom: 50px;
    }

    /* KPI Card เน้นตัวเลขใหญ่พิเศษ */
    .kpi-card {
        background: #f8fafc; padding: 3rem; border-radius: 30px; text-align: center;
        box-shadow: 0 4px 20px rgba(0,0,0,0.05); border: 2px solid #e2e8f0;
    }
    .kpi-label { color: #475569; font-size: 1.3rem; font-weight: 700; text-transform: uppercase; }
    .kpi-value { color: #000000; font-size: 4.5rem; font-weight: 800; margin: 0.5rem 0; line-height: 1; }
    .kpi-delta-box { padding: 12px 24px; border-radius: 50px; display: inline-block; font-weight: 800; font-size: 1.6rem; }

    /* ปุ่มกดตัวหนาชัดเจน */
    .stButton>button {
        border-radius: 15px; background: #0f172a; color: white;
        font-weight: 800; border: none; padding: 1.2rem 2rem; font-size: 1.2rem;
    }
    </style>
...
def color_growth(val):
    ...
    return f'color: {color}; font-weight: 800; font-size: 1.1rem;'
...
if file_1 and file_2:
    ...
    if page == "📊 Executive Dashboard":
        ...
        with c2:
            st.markdown(f"<div class='kpi-card'><p class='kpi-label'>Current Total</p><p class='kpi-value' style='color:#2563eb;'>{t2:,}</p></div>", unsafe_allow_html=True)
        with c3:
            st.markdown(f"""
                <div class='kpi-card'>
        ...
        fig_bar = px.bar(df_compare, x='Ward', y=['Total_Days_M1', 'Total_Days_M2'],
                         ...
                         labels={'value': 'Days', 'variable': 'Month'})
        fig_bar.update_layout(
            font=dict(size=18, family="Sarabun", color="#000000"),
            plot_bgcolor='rgba(0,0,0,0)', 
            xaxis=dict(tickfont=dict(size=18, color="#000", weight='bold')),
            legend=dict(font=dict(size=16), orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
        )
        fig_bar.update_traces(textposition='outside', textfont_size=20, textfont_weight="bold")
        st.plotly_chart(fig_bar, use_container_width=True)

        # --- กราฟที่ 2: Growth % ชัดเจน ---
        st.subheader("📈 อัตราการเติบโตแยกแผนก (%)")
        ...
        fig_growth.update_traces(textposition='outside', textfont_size=20, textfont_weight="bold")
        st.plotly_chart(fig_growth, use_container_width=True)

        # ตารางสรุป (ตัวหนังสือชัดเจน)
        st.markdown("---")
        st.subheader("📋 ตารางข้อมูลสรุปผู้บริหาร")
        ...
        st.download_button("📥 ดาวน์โหลดรายงานสรุป (Excel)", data=buf_comp.getvalue(), file_name="Executive_Summary.xlsx")
        ...
        if device_cols:
            ...
            if st.button("📥 รวบรวมทุกวอร์ดเป็นไฟล์เดียว"):
                with st.spinner('กำลังประมวลผล...'):
                    ...
                    st.download_button("📥 คลิกเพื่อดาวน์โหลดรายงานโรงพยาบาล", data=bulk_buf.getvalue(), file_name="Full_Hospital_Report.xlsx")
