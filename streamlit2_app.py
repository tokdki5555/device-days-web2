...
st.markdown("""
    ...
    .kpi-label { color: #64748b; font-size: 1rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
    .kpi-value { color: #1e293b; font-size: 3.5rem; font-weight: 800; margin: 0.5rem 0; }
    .kpi-delta-box { padding: 8px 16px; border-radius: 12px; display: inline-block; font-weight: 800; font-size: 1.2rem; }
...
if file_1 and file_2:
    st.sidebar.markdown("---")
    page = st.sidebar.selectbox("เลือกหน้าจอ", ["📊 Comparison Dashboard", "📄 Interactive Editor"])
    ...
    # คำนวณส่วนต่างและ % Growth (Integer Only)
    ...
    if page == "📊 Comparison Dashboard":
        st.markdown("<h1 class='main-title'>Device Usage Insights</h1>", unsafe_allow_html=True)
        ...
        with col_left:
            st.subheader("📊 จำนวนวันการใช้งาน (M1 vs M2)")
            ...
            fig_bar.update_traces(textposition='outside', textfont_size=14, textfont_color="#000")
            st.plotly_chart(fig_bar, use_container_width=True)

        with col_right:
            st.subheader("📈 อัตราการเติบโตรายแผนก (%)")
            # ใส่สีตามเงื่อนไข: เขียวถ้าโต แดงถ้าลด
            df_compare['Status'] = df_compare['% Growth'].apply(lambda x: 'โตขึ้น' if x >= 0 else 'ลดลง')
            fig_growth = px.bar(df_compare, x='% Growth', y='Ward', orientation='h',
                                color='Status', color_discrete_map={'โตขึ้น': '#10b981', 'ลดลง': '#ef4444'},
                                text_auto='d')
            fig_growth.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', showlegend=False)
            fig_growth.update_traces(textposition='outside', textfont_size=14, textfont_weight='bold')
            st.plotly_chart(fig_growth, use_container_width=True)

        # ตารางสรุปข้อมูล
        st.markdown("---")
        st.subheader("📋 ตารางสรุปผลการดำเนินงานสุทธิ")
        ...
        st.download_button("📥 Export Detailed Report (Excel)", data=buf_comp.getvalue(), file_name="Executive_Summary.xlsx")

    elif page == "📄 Interactive Editor":
        excel_2 = pd.ExcelFile(file_2)
        selected_sheet = st.sidebar.selectbox("เลือกแผนกที่ต้องการแก้ไข:", excel_2.sheet_names)
        ...
        if device_cols:
            ...
            st.markdown("---")
            if st.button("📥 เตรียมไฟล์รวมทุกแผนกสำหรับดาวน์โหลด"):
                with st.spinner('กำลังรวบรวมข้อมูลทุกแผนก...'):
                    bulk_buf = io.BytesIO()
                    ...
                    st.download_button("📥 คลิกเพื่อดาวน์โหลด (รวมทุก Sheet)", data=bulk_buf.getvalue(), file_name="Full_Hospital_Report.xlsx")
        else:
            st.warning("ไม่พบคอลัมน์อุปกรณ์ในแผนกนี้")

else:
    st.info("กรุณาอัปโหลดไฟล์ Excel ทั้ง 2 ช่วงเวลาที่แถบด้านซ้ายเพื่อเริ่มการวิเคราะห์")
